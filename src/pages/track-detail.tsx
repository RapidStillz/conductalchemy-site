...
import { getTrack, Track, AccessStatus, type SocialLinks } from "@/lib/cms";
...
function SocialLinksPanel({ links, art }: { links: SocialLinks; art: boolean }) {
  const entries = (Object.keys(links) as (keyof SocialLinks)[]).filter(
    (k) => !!links[k]
  );
  ...
  return (
    <section>
      <h2
        className={`text-xs font-sans tracking-[0.2em] uppercase mb-5 border-b pb-4 ${
          art ? "text-[#1a1510]/50 border-[#1a1510]/15" : "text-muted-foreground border-border/40"
        }`}
      >
        Find Us On
      </h2>
        ...
        {entries.map((key) => (
          <a
            ...
            className={`text-[10px] font-sans tracking-widest uppercase border px-3 py-2 transition-colors flex items-center gap-2 ${
              art
                ? "border-[#1a1510]/20 text-[#2a1f10] hover:border-[#b5a882] hover:bg-[#ede5d0]"
...
export default function TrackDetail() {
  ...
  const embedUrl = track.videoUrl ? getEmbedUrl(track.videoUrl) : null;
  const hasSocialLinks =
    track.socialLinks && Object.values(track.socialLinks).some(Boolean);
    ...
    <>
      ...
      <div className={`min-h-screen transition-colors duration-700 ${art ? "bg-[#f5f0e8] text-[#1a1510]" : ""}`}>
        <div className="container mx-auto px-4 md:px-8 py-12">
          ...
          {/* Top Bar */}
          <div
            className="flex flex-wrap justify-between items-center mb-16 border-b pb-6 gap-4"
            ...
            <div className="flex items-center gap-4">
              <Link
                ...
                className={`text-[10px] font-sans tracking-[0.2em] uppercase transition-colors ${
                  art ? "text-[#1a1510]/50 hover:text-[#1a1510]" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                ← Catalogue
              </Link>
              <span className={`text-[10px] font-sans ${art ? "text-[#1a1510]/30" : "text-border"}`}>/</span>
              <span className={`text-[10px] font-sans ${art ? "text-[#1a1510]/50" : "text-muted-foreground"}`}>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            {track.coverArtUrl && (
              <div className="mb-12 relative overflow-hidden">
                ...
                <div
                  ...
                  style={{
                    background: art
                      ? "linear-gradient(to bottom, transparent 50%, rgba(245,240,232,0.8))"
                      : "linear-gradient(to bottom, transparent 50%, rgba(13,13,13,0.8))",
                  }}
                />
              </div>
            )}
            ...
            <div className="mb-16 text-center flex flex-col items-center">
              <div className={`text-sm font-sans tracking-[0.3em] uppercase mb-6 ${art ? "text-[#5c4a28]" : "text-primary"}`}>
                {track.genre}
              </div>
              <h1 className={`text-5xl md:text-7xl font-serif mb-8 leading-tight ${art ? "tracking-wide text-[#1a1510]" : "tracking-tight"}`}>
                {track.title}
              </h1>
              <div className={`flex flex-wrap justify-center gap-6 text-xs font-sans tracking-widest uppercase border-t border-b py-4 w-full max-w-2xl ${art ? "border-[#1a1510]/15 text-[#1a1510]/60" : "border-border/40 text-muted-foreground"}`}>
                ...
                {track.collaborators && track.collaborators.length > 0 && (
                  <div>
                    <span className="opacity-50 mr-2">With</span>
            ...
            {embedUrl && (!isPrivate || unlocked) && (
              <div className="mb-16">
                <h2 className={`text-xs font-sans tracking-[0.2em] uppercase mb-6 ${art ? "text-[#1a1510]/50" : "text-muted-foreground"}`}>
                  Video
                </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-16 lg:gap-24">
              ...
              <div className="md:col-span-7 lg:col-span-8 space-y-16">
                <section>
                  ...
                  <p className={`text-xl md:text-2xl font-serif leading-relaxed italic ${art ? "text-[#2a1f10]" : ""}`}>
                    {track.description}
                  </p>
                </section>
                ...
                {track.lyrics && track.lyrics.trim() !== "(Instrumental)" && (
                  <section>
                    ...
                    <div className={`font-serif text-lg leading-loose whitespace-pre-wrap pl-6 border-l ${art ? "border-[#b5a882] text-[#2a1f10]" : "border-primary/30"}`}>
                      {isPrivate && !unlocked ? (
                        <span className={`text-sm italic ${art ? "text-[#1a1510]/40" : "text-muted-foreground/50"}`}>
                ...
                {track.visualConceptNotes && (
                  <section className={`border p-8 md:p-10 ${art ? "border-[#1a1510]/15 bg-[#ede5d0]" : "bg-card/30 border-border/40"}`}>
                    ...
                    <p className={`text-lg font-serif italic leading-relaxed ${art ? "text-[#3a2e1e]" : "text-muted-foreground"}`}>
                      "{track.visualConceptNotes}"
                    </p>
                  </section>
                )}
              ...
              <div className="md:col-span-5 lg:col-span-4 space-y-10">
                ...
                <section>
                  ...
                  {isPrivate && unlocked && (
                    <p className={`text-xs font-sans mt-3 leading-relaxed ${art ? "text-[#5c4a28]" : "text-green-400"}`}>
                ...
                <section>
                  ...
                  <ul className="space-y-4">
                    {track.versions.map((v, i) => (
                      <li key={i} className={`flex items-center gap-3 text-sm font-sans tracking-wide ${art ? "text-[#2a1f10]" : ""}`}>
                        ...
                        {isPrivate && !unlocked ? (
                          <span className={art ? "text-[#1a1510]/30" : "text-muted-foreground/30"}>
                  ...
                  {isPrivate && !unlocked && (
                    <button
                      ...
                      className={`mt-4 text-[10px] font-sans tracking-[0.2em] uppercase ${art ? "text-[#5c4a28]" : "text-primary"} hover:underline`}
                    >
                ...
                <section>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {track.mood.map((m) => (
                      <span key={m} className={`text-[10px] uppercase tracking-widest border px-3 py-1.5 ${art ? "border-[#1a1510]/20 text-[#2a1f10]" : "border-border/60 text-foreground"}`}>
                        {m}
                      </span>
                    ))}
                  </div>
